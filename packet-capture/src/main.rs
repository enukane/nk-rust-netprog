#[macro_use]
extern crate log;
extern crate pnet;

use std::env;
use pnet::datalink::{self, NetworkInterface, Channel::Ethernet};
use pnet::packet::Packet;
use pnet::packet::ethernet::{EthernetPacket, EtherTypes};
use pnet::packet::ip::IpNextHeaderProtocols;
use pnet::packet::ipv4::Ipv4Packet;
use pnet::packet::ipv6::Ipv6Packet;
use pnet::packet::tcp::TcpPacket;
use pnet::packet::udp::UdpPacket;
use pnet::packet::icmp::IcmpPacket;
//use packets;
mod packets;
use crate::packets::GettableEndPoints;

const WIDTH: usize = 16;

fn main() {
    env::set_var("RUST_LOG", "debug");
    env_logger::init();
    let args: Vec<String> = env::args().collect();

    if args.len() != 2 {
        println!("Specify target interface name");
        std::process::exit(1);
    }

    let interface_name = &args[1];

    let interfaces = datalink::interfaces();

    println!("[Interfaces]");
    for interface in &interfaces {
        println!("  - name:{:10} idx:{:<5} macaddr:{:}", interface.name, interface.index, interface.mac.unwrap());
    }

    let interface = interfaces
        .into_iter()
        .find(|iface| iface.name == *interface_name)
        .expect("Failed to get interface");

    let (_tx, mut rx) = match datalink::channel(&interface, Default::default()) {
        Ok(Ethernet(tx, rx)) => (tx, rx),
        Ok(_) => panic!("Unhandled channel type"),
        Err(e) => panic!("Failed to create datalink channel {}", e),
    };

    loop {
        match rx.next() {
            Ok(frame) => {
                let frame = EthernetPacket::new(frame).unwrap();
                match frame.get_ethertype() {
                    EtherTypes::Ipv4 => {
                        ipv4_handler(&frame);
                    }
                    EtherTypes::Ipv6 => {
                        ipv6_handler(&frame);
                    }
                    _ => {
                        info!("Not an IPv4 or IPv6 packet");
                    }
                }
            }
            Err(e) => {
                error!("Failed to read {}", e);
            }
        }

    }
}

fn ipv4_handler(ethernet: &EthernetPacket) {
    if let Some(packet) = Ipv4Packet::new(ethernet.payload()) {
        match packet.get_next_level_protocol() {
            IpNextHeaderProtocols::Tcp => {
                tcp_handler(&packet);
            }
            IpNextHeaderProtocols::Udp => {
                udp_handler(&packet);
            }
            IpNextHeaderProtocols::Icmp => {
                icmp_handler(&packet);
            }
            _ => {
                info!("Not a TCP/UDP/ICMP packet");
            }
        }
    }
}

fn ipv6_handler(ethernet: &EthernetPacket) {
    if let Some(packet) = Ipv6Packet::new(ethernet.payload()) {
        match packet.get_next_header() {
            IpNextHeaderProtocols::Tcp => {
                tcp_handler(&packet);
            }
            IpNextHeaderProtocols::Udp => {
                udp_handler(&packet);
            }
            IpNextHeaderProtocols::Icmpv6 => {
                //icmpv6_handler(&packet);
            }
            _ => {
                info!("Not a TCP/UDP/ICMPv6 packet");
            }
        }
    }
}

fn tcp_handler(packet: &GettableEndPoints) {
    let tcp = TcpPacket::new(packet.get_payload());
    if let Some(tcp) = tcp {
        print_packet_info(packet, &tcp, "TCP");
    }
}

fn udp_handler(packet: &GettableEndPoints) {
    let udp = UdpPacket::new(packet.get_payload());
    if let Some(udp) = udp {
        print_packet_info(packet, &udp, "UDP");
    }
}

fn icmp_handler(packet: &GettableEndPoints) {
    let icmp = IcmpPacket::new(packet.get_payload());
    if let Some(icmp) = icmp {
        print_packet_info(packet, &icmp, "ICMP");
    }
}

fn print_packet_info(l3: &GettableEndPoints, l4: &GettableEndPoints, proto: &str) {
    let payload = l4.get_payload();
    let len = payload.len();

    println!("Captured a {} packet (length: {} bytes) from {}|{} to {}|{}",
            proto,
            len,
            l3.get_source(),
            l4.get_source(),
            l3.get_destination(),
            l4.get_destination()
            );

    println!("{}", "=".repeat(WIDTH * 3));
    for i in 0..len {
        print!("{:<02x} ", payload[i]);
        if i % WIDTH == WIDTH - 1 || i == len - 1 {
            for _j in 0..WIDTH - 1 - (i % (WIDTH)) {
                print!("   ");
            }
            print!("| ");
            for j in i - i % WIDTH..=i {
                if payload[j].is_ascii_alphabetic() {
                    print!("{}", payload[j] as char);
                } else {
                    print!(".");
                }
            }
            println!();
        }
    }
    println!("{}", "=".repeat(WIDTH * 3));
    println!();
}